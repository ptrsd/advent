...
class a72 {
    record bag(String name, int number){}
...
drab aqua bags contain 3 bright lime bags, 5 posh red bags.
        ...
        Map<String, List<bag>> collect = asList(lines).stream()
                .map(line -> line.split(" bags contain "))
                .collect(Collectors.toMap(x -> x[0].trim(),
                        x -> {
                            if (x[1].trim().contains("no other bags")) {
                                return new ArrayList<>();
                            } else {
                                String[] split = x[1].trim().split(",");
                                return Arrays.asList(split).stream()
                                        .map(s -> s.replaceAll("bags.|bag.", "").trim())
                                        .filter(s ->!s.isBlank())
                                        .map(s -> s.split(" "))
                                        .map(s -> new bag(String.format("%s %s", s[1], s[2]), Integer.valueOf(s[0])))
        ...
        int count = find(collect, "shiny gold");
        System.out.println(count - 1);
    }

    public static int find(Map<String, List<bag>> rules, String current) {
        int result = 1;
        for (bag bag : rules.get(current)) {
            result += bag.number * find(rules, bag.name);
        }
        return result;
    }
}