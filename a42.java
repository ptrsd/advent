...
iyr:2014 pid:896056539 hcl:#a97842 hgt:165cm
    ...
    Predicate<String> cmPredicate = (s) -> {
      String cm = s.replace("cm", "");
      return Integer.valueOf(cm) >= 150 && 193 >= Integer.valueOf(cm);
    };

    Predicate<String> inPredicate = (s) -> {
      String in = s.replace("in", "");
      return Integer.valueOf(in) >= 59 && 76 >= Integer.valueOf(in);
    };

    HashMap<String, Predicate<String>> fields = new HashMap<>();
    fields.put("byr", (str) -> Integer.valueOf(str) >= 1920 && 2002 >= Integer.valueOf(str));
    fields.put("iyr", (str) -> Integer.valueOf(str) >= 2010 && 2020 >= Integer.valueOf(str));
    fields.put("eyr", (str) -> str.length() == 4 && Integer.valueOf(str) >= 2020 && 2030 >= Integer.valueOf(str));
    fields.put("hgt", (str) -> str.toLowerCase().endsWith("cm") && cmPredicate.test(str) || str.toLowerCase().endsWith("in") && inPredicate.test(str));
    fields.put("hcl", (str) -> str.startsWith("#") && str.length() == 7);
    fields.put("ecl", (str) -> Set.of("amb", "blu", "brn", "gry", "grn", "hzl", "oth").contains(str));
    fields.put("pid", (str) -> str.matches("[0-9]{9}"));
    ...
    for (int i = 0; i < lines.length; i++) {
      ...
      if (lines[i].isBlank() || i + 1 == lines.length) {
        ...
        if (map.keySet().containsAll(fields.keySet()) &&
            map.entrySet().stream().allMatch(s -> fields.getOrDefault(s.getKey(), (str) -> true).test(s.getValue()))) {
          count++;
        }
