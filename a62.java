...
public class a62 {
  ...
  public static void main(String[] args) {
    ...
    Long sumOfAnswers = 0L;
    List<String> accumulator = new ArrayList<>();
    for (int i = 0; i < lines.length; i++) {
      accumulator.add(lines[i]);
      if (lines[i].isBlank() || i + 1 == lines.length) {
        final Long groupAnswersNo = accumulator.stream().filter(s -> !s.isBlank()).count();
        List<String> answers = List.of(accumulator.stream().reduce("", (acc, cur) -> acc.concat(cur)).split(""));
        Map<String, Long> lettersRank = answers.stream()
          .collect(groupingBy(identity(), counting()));

        sumOfAnswers += lettersRank.entrySet().stream()
          .filter((e) -> valueOf(e.getValue()) / groupAnswersNo >= 1L)
          .count();
