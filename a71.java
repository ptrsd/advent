...
drab aqua bags contain 3 bright lime bags, 5 posh red bags.
        ...
        Map<String, List<String>> collect = asList(lines).stream()
                .map(line -> line.split(" bags contain "))
                .filter(x -> !x[0].contains("shiny gold"))
                .collect(Collectors.toMap(x -> x[0].trim(),
                        x -> {
                            if (x[1].trim().contains("no other bags")) {
                                return new ArrayList<>();
                            } else {
                                String[] split = x[1].trim().split("[0-9]");
        ...
        int count = 0;
        for (String bag : collect.keySet()) {
            if (find(collect, bag, "shiny gold")) count++;
        }

        System.out.println(count);
    }

    public static boolean find(Map<String, List<String>> rules, String current, String goal) {
        if (goal.equals(current)) return true;
        for (String bag : rules.get(current)) {
            if (find(rules, bag, goal)) return true;
        }
        return false;
    }
}