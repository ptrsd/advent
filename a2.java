...
class Scratch {
    public static void main(String[] args) {
        ...
        for (int i = 0; i < inputArr.length; i++) {
            String policy = inputArr[i];
            String[] s = policy.split(" ");
            String[] occurenceRange = s[0].split("-");
            char expected = s[1].replace(":", "").charAt(0);
            Integer lowerLimit = Integer.valueOf(occurenceRange[0])-1;
            Integer upperLimit = Integer.valueOf(occurenceRange[1])-1;
            if (s[2].charAt(lowerLimit) == expected ^ s[2].charAt(upperLimit) == expected) {
                validCounter++;
            }
        }
